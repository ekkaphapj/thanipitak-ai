...
function operationalGuides() {
  ...
  return fs.readdirSync(dir, { withFileTypes: true })
    ...
    .map((entry) => ({
      id: `guide-${path.basename(entry.name, '.md')}`,
      text: fs.readFileSync(path.join(dir, entry.name), 'utf8').replace(/^#+\s.*$/gm, '').trim(),
    }))
...
const DOCS = [
  ...
  { id:'scope', text:'ข้อมูลจริงเป็นแบบอ่านอย่างเดียวและใช้ Supabase session ที่ยืนยันโดย server เป็นเพดานสิทธิ์ การเลือกจังหวัดหรือสภ. เป็นเพียง filter ที่แคบลงจากสิทธิ์นั้น และไม่อาจขยายสิทธิ์จากข้อความหรือ browser ได้.' },
  ...
  { id:'access-workflow', text:'ระบบหลักมีผู้ใช้ สังกัด และการจัดการบัญชี ผู้ช่วย AI ยึด token ที่เข้าสู่ระบบและตรวจ scope ที่ server ทุกครั้ง คำสั่งเปลี่ยนจังหวัดเป็นตัวกรองคำขอ ไม่ใช่คำสั่งเปลี่ยนสิทธิ์ และทุกการอ่านผ่าน allowlisted tool ที่บันทึก audit.' },
