...
- This repository is not yet ready for general production distribution. Real registry facts, recorded visit/alert fields, and station-scoped PDF/Excel (no id_card/phones) exist. Do not claim time-window monitoring or production packaging is complete. Authenticated police users may read authorized registry data across every station and province through server-verified Supabase sessions only. Every real-data read must be recorded in an audit log with the actor, timestamp, request type, requested province/station, and result metadata. The AI may only use allowlisted, read-only server/RLS operations and must never use model output, request-body scope, fixture accounts, or a service-role key to expand access. Never copy fixture monitoring thresholds onto real rows. Never copy fixture monitoring thresholds onto real rows.
- Sensitive fields may be returned only through authenticated, audited, allowlisted real-data tools. Write permissions remain separate from read permissions; AI tools must not create, update, or delete registry records.
