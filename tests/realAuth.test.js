...
test('real login verifies credentials remotely then reads only the authenticated profile',async()=>{
 ...
 app.use(createRealAuthRoutes({url:'https://example.test',key:'public-test',request:async(url,opts)=>{
  calls.push({url,opts});
  return {ok:true,json:async()=>url.includes('/token?')?{access_token:'real-token'}:url.endsWith('/user')?{id:'uuid'}:url.includes('/ai-access-scope')?{scope:{level:'region4',read_only:true,user_id:8,provinces:['นครพนม']}}:url.includes('/stations?')?[{station_id:2,station_name:'สภ.ท่าอุเทน',division:'ภ.จว.นครพนม',province:'นครพนม'}]:[{user_id:8,username:'4648',name:'Example',user_type:'User',station_id:2}]};
 }}));
 ...
 assert.equal(res.body.user.aiScope.level,'region4');
 assert.equal(calls[4].opts.headers.Authorization,'Bearer real-token');
